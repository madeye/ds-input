#!/usr/bin/env python3
"""
Build the seed corpus for the local speculative n-gram model from open-source
Chinese frequency data, and emit it as TSV on stdout so it can be baked straight
into the embedded binary baseline (core/data/seed_model.bin). There is no longer
a committed plain-text corpus file: the TSV is an ephemeral pipeline stage.
...
Regenerate the embedded baseline (run from the repo root):
    python3 core/data/gen_corpus.py \\
        | cargo run --release --example bake_seed -- core/data/seed_model.bin

Or capture the intermediate TSV for inspection:
    python3 core/data/gen_corpus.py seed.tsv
"""

import urllib.request
import re, math, sys, functools
from pathlib import Path
from collections import defaultdict
from pypinyin import lazy_pinyin, Style

# All progress/diagnostic output goes to stderr; stdout is reserved for the
# corpus itself so `gen_corpus.py | bake_seed` works as a pipeline.
print = functools.partial(print, file=sys.stderr)  # noqa: A001

# ── Config ────────────────────────────────────────────────────────────────────

# Output path from argv[1]; "-" (the default) writes the TSV corpus to stdout so
# it can be piped straight into `cargo run --example bake_seed`.
...
# ── 4. Emit TSV (stdout by default, or argv[1]) ───────────────────────────────
...
#
# Generated by core/data/gen_corpus.py; baked into seed_model.bin via bake_seed.
"""
...
print(f"\nWritten {len(rows)} entries → {dest}", flush=True)
...
print(f"Corpus size: {size_kb} KB")
